...
export default function AdminUsers() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedUser || !messageForm.title || !messageForm.content) return;
    ...
    try {
      await addDoc(collection(db, 'user_messages'), {
        ...
        code: messageForm.type !== 'message' ? messageForm.code : '',
        senderId: 'Admin',
      ...
      setToast({
        isVisible: true,
        message: `Message dispatched to student ${selectedUser.name}`,
        type: 'success'
      });
      ...
      setToast({
        isVisible: true,
        message: 'Could not dispatch message. Network protocol failed.',
        type: 'error'
      });
    } finally {
  ...
  return (
    <AdminLayout title="User Management">
                  ...
                  <tr key={user.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="p-4 pr-6 text-right">
                       <div className="flex items-center justify-end gap-2 text-slate-400">
                          ...
                          <button onClick={() => { setSelectedUser(user); setMessageForm({ title: '', content: '', type: 'message', code: '' }); setShowMessageModal(true); }} className="p-2 hover:bg-emerald-50 hover:text-emerald-700 rounded text-emerald-600 transition-colors" title="Send Message & Vouchers">
      ...
      {showMessageModal && selectedUser && (
        <div className="fixed inset-0 bg-[#002f26]/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-200">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-slate-50/50">
               <div>
                  <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <MessageSquare className="w-5 h-5 text-[#006e5d]" />
                    Send Direct Message & Vouchers
                  </h2>
                  <p className="text-sm text-slate-500">To: {selectedUser.name} ({selectedUser.email})</p>
               </div>
               <button onClick={() => setShowMessageModal(false)} className="p-2 bg-white border border-slate-200 rounded-md text-slate-400 hover:text-slate-600 transition-all">
                 <XCircle className="w-5 h-5" />
               </button>
            </div>
            <form onSubmit={handleSendMessage} className="p-6 space-y-4">
                <div>
                   <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-slate-500">Message Type</label>
                   <select 
                     value={messageForm.type} 
                     onChange={e => setMessageForm({...messageForm, type: e.target.value as any})}
                     className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#006e5d] focus:border-transparent bg-white text-sm font-semibold text-slate-950"
                   >
                     <option value="message">Direct Text Message</option>
                     <option value="voucher">Gift Voucher</option>
                     <option value="coupon">Discount Coupon</option>
                   </select>
                </div>
                <div>
                   <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-slate-500">Subject / Title</label>
                   <input 
                     required 
                     placeholder={messageForm.type === 'message' ? "e.g., Exam Prep Update" : "e.g., Double Cash Coupon Code 🎁"}
                     className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#006e5d] focus:border-transparent text-sm font-semibold text-slate-950" 
                     value={messageForm.title} 
                     onChange={e => setMessageForm({...messageForm, title: e.target.value})} 
                   />
                </div>
                <div>
                   <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-slate-500">Narration / Message Body</label>
                   <textarea 
                     required 
                     rows={4}
                     placeholder="Type your instruction or offer details here..."
                     className="w-full px-4 py-3 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#006e5d] focus:border-transparent text-sm font-medium resize-none leading-relaxed text-slate-950" 
                ...
                {messageForm.type !== 'message' && (
                  <div>
                     <label className="block text-xs font-bold uppercase tracking-wider mb-1 text-slate-500 flex items-center gap-1">
                       <Ticket className="w-3.5 h-3.5" /> Coupon / Voucher Code
                     </label>
                     <input 
                       required 
                       placeholder="e.g. GETPASS50 or GIFT-XYZ-123"
                       className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#006e5d] focus:border-transparent text-sm font-bold uppercase placeholder:normal-case text-slate-950" 
                       value={messageForm.code} 
                       onChange={e => setMessageForm({...messageForm, code: e.target.value})} 
                     />
                  </div>
                )}
                
                <div className="pt-6 flex justify-end gap-3 border-t border-slate-100">
                   <button type="button" onClick={() => setShowMessageModal(false)} className="px-5 py-2.5 bg-slate-100 rounded-lg text-slate-700 font-semibold hover:bg-slate-200 transition-colors text-sm">Cancel</button>
                   <button type="submit" disabled={sendingMessage} className="px-5 py-2.5 bg-[#006e5d] text-white rounded-lg font-bold hover:bg-[#005a4d] transition-colors flex items-center gap-1.5 text-sm disabled:opacity-50">
